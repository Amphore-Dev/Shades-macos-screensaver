PROJECT   = Shades.xcodeproj
SCHEME    = Shades
BUILD_DIR = build

# Recursive header search — allows flat #import "KGBaseShape.h" from any subfolder
HEADERS = HEADER_SEARCH_PATHS='$$(inherited) $$(SRCROOT)/Shades/**'
...
install: release
	...
	rm -rf ~/Library/Screen\ Savers/Shades.saver
	@# Copier le nouveau bundle
	cp -R $(BUILD_DIR)/Build/Products/Release/Shades.saver \
		~/Library/Screen\ Savers/
	@# Re-signer ad-hoc pour garantir une signature fraîche
	codesign --force --deep -s - ~/Library/Screen\ Savers/Shades.saver
